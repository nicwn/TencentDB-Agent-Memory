...
AGENT_LABELS=(
  ...
  "Pi                — OpenAI Chat + 扩展交互式 Picker, ~/.pi/agent/extensions/pi-tdai-client"
)
...
DEFAULT_CONFIG_PATHS=(
  ...
  "~/.pi/agent/extensions/pi-tdai-client"
)
...
if $USE_SCANNED && [[ -n "$MODEL_ID" ]]; then
  ...
  case "$CHOSEN_AGENT" in
    ...
    pi)          DEFAULT_MODEL="glm-5.2-vision" ;;
...
write_pi() {
  local ext_dir="$1"
  ensure_dir "$ext_dir"
  local env_file="${ext_dir}.env"

  # Install the extension: symlink the repo's pi-plugin into ~/.pi/agent/extensions/.
  # Fallback: copy if symlink fails (e.g. cross-filesystem). Idempotent.
  local repo_root plugin_src
  repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
  plugin_src="${repo_root}/MemoryCore/pi-plugin"
  if [[ ! -f "${plugin_src}/index.ts" ]]; then
    warn "未找到 ${plugin_src}/index.ts（仓库结构变了？）—— 跳过扩展安装，请手动软链/复制 pi-plugin"
  else
    if [[ -L "$ext_dir" || -d "$ext_dir" ]]; then
      info "扩展目录已存在，跳过安装（如需更新请删除后重跑）: ${ext_dir}"
    else
      mkdir -p "$(dirname "$ext_dir")"
      if ln -s "$plugin_src" "$ext_dir" 2>/dev/null; then
        success "已软链扩展: ${ext_dir} -> ${plugin_src}"
      else
        cp -R "$plugin_src" "$ext_dir"
        success "已复制扩展到: ${ext_dir}（软链失败，改用复制）"
      fi
    fi
  fi

  # Write a sourced env file. Pi has no config file; env vars are the config.
  # Interactive mode (default): omit TDAI_TEAM_ID/TDAI_AGENT_ID → picker on turn 1.
  # Static mode: include them when passed via --team-id/--agent-id flags.
  cat > "$env_file" <<EOF
# Generated by setup-proxy.sh at $(date '+%Y-%m-%d %H:%M:%S')
# Source before launching Pi:  source ${env_file}
export TDAI_PROXY_URL="${PROXY_HOST}"
export TDAI_SPACE_ID="${INSTANCE_ID}"
export TDAI_USER_KEY="${USER_KEY}"
export TDAI_MODEL="${MODEL_ID}"
export TDAI_AGENT_SOURCE="pi"
EOF
  # Static preset (optional) — only when explicitly provided via flags.
  if [[ -n "$TEAM_ID" && -n "$AGENT_ID" ]]; then
    cat >> "$env_file" <<EOF
export TDAI_TEAM_ID="${TEAM_ID}"
export TDAI_AGENT_ID="${AGENT_ID}"
EOF
    [[ -n "$TASK_ID" && "$TASK_ID" != "no-task" ]] && echo "export TDAI_TASK_ID=\"${TASK_ID}\"" >> "$env_file"
    info "已写入静态预选 (TDAI_TEAM_ID/TDAI_AGENT_ID) → 跳过首轮 picker"
  else
    info "未设静态预选 → 首轮将弹 Team→Agent→Task 交互式选择菜单"
  fi

  success "已写入 ${env_file}"
  chmod 600 "$env_file"  # contains TDAI_USER_KEY (a secret) — restrict read access
  echo ""
  info "启动方式:"
  echo -e "  ${GREEN}source ${env_file}${RESET}"
  echo -e "  ${GREEN}pi --provider tdai --model ${MODEL_ID}${RESET}"
  echo -e "  ${DIM}扩展位置: ${ext_dir}（Pi 启动时自动加载 ~/.pi/agent/extensions/ 下的扩展）${RESET}"
}
...
case "$CHOSEN_AGENT" in
  ...
  hermes|openclaw)
    ...
    ;;
  pi)
    warn "使用时运行: ${BOLD}source ~/.pi/agent/extensions/pi-tdai-client.env${RESET} 然后运行 ${BOLD}pi --provider tdai --model ${MODEL_ID}${RESET}"
    echo -e "  ${DIM}默认交互模式（不设 TEAM/AGENT）首轮会弹 Team→Agent→Task 选择菜单${RESET}"
    echo -e "  ${DIM}静态预选（跳过菜单）: 在 .env 里设 TDAI_TEAM_ID + TDAI_AGENT_ID${RESET}"
    ;;
esac
echo ""
